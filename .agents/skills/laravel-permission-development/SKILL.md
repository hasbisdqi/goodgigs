...
## Frontend Permission Checking (React/Inertia)

Always use the custom `usePermission` React hook to perform role and permission checks on the frontend. Do NOT manually check `auth.permissions` or `auth.roles` from page props to ensure consistent Super Admin gate bypass logic.
...
export default function MyComponent() {
    ...
    return (
        <div>
            {hasPermission('manage users') && <button>Manage Users</button>}
        </div>
    );
}
```
